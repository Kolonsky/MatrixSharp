...
namespace MatrixSharp.Api.EventSchemas.Schema
{
	...
	// 
	// .. Note::
	// Server ACLs do not restrict the events relative to the room DAG via authorization
	...
	public record RoomServerAclEvent : StateEvent<RoomServerAclEvent.ContentProperty>
	{
		private const string TYPE = "m.room.server_acl";

		/// <inheritdoc cref="RoomServerAclEvent" />
		/// <param name="stateKey"> A zero-length string.</param>
		public RoomServerAclEvent(ContentProperty content, string eventId, string sender, long originServerTs,
		...
		public record ContentProperty : IContentProperty
		{
			...
			/// </remarks>
			public bool? AllowIpLiterals { get; init; }
			...
			///     matches zero or more characters and ``?`` matches exactly one character.
			/// </summary>
			/// <remarks>This defaults to an empty list when not provided.</remarks>
