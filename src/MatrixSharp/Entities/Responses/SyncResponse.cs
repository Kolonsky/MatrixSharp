...
namespace MatrixSharp.Entities.Responses
{
	...
	public class Rooms
	{
		/// <inheritdoc cref="JoinedRooms"/>
		[JsonPropertyName("join")]
		public JoinedRooms? Join { get; set; }

		/// <inheritdoc cref="InvitedRooms"/>
		[JsonPropertyName("invite")]
		public InvitedRooms? Invite { get; set; }

		/// <inheritdoc cref="LeftRooms"/>
		[JsonPropertyName("leave")]
		public LeftRooms? Leave { get; set; }
	}
	...
	public class JoinedRooms
	{
		...
		/// </summary>
		/// <remarks>State updates for m.room.member events will be incomplete if lazy_load_members is enabled in the /sync filter, and only return the member events required to display the senders of the timeline events in this response.</remarks>
	...
	{
		/// <inheritdoc cref="Responses.StrippedState"/>
	...
	public class RoomSummary
	{
		...
		/// </summary>
		/// <remarks>This should be the first 5 members of the room, ordered by stream ordering, which are joined or invited. The list must never include the client's own user ID. When no joined or invited members are available, this should consist of the banned and left users. More than 5 members may be provided, however less than 5 should only be provided when there are less than 5 members to represent.
		/// When lazy-loading room members is enabled, the membership events for the heroes MUST be included in the state, unless they are redundant. When the list of users changes, the server notifies the client by sending a fresh list of heroes. If there are no changes since the last sync, this field may be omitted.</remarks>
